...
class JobRunner:
    ...
    def run_once(self, db: Session) -> dict[str, object]:
        active_count = db.query(Job).filter(
            ...
            Job.status.in_(["reserved", "booting_browser", "logging_in", "running", "uploading_result"]),
        ).count()
        ...
        ).order_by(Job.priority.asc(), Job.created_at.asc()).first()
        if job is None:
            return {
                "worker_id": self.config.worker_id,
                "status": "idle",
                "detail": "No queued job available",
            }
    ...
    def _execute_job(self, db: Session, job: Job) -> None:
        ...
        provider = job.provider or profile.category
        execution_context = ProviderExecutionContext(
            job_id=job.id,
            profile_id=profile.id,
            profile_name=profile.name,
            provider=provider,
            job_type=job.job_type or "generate_image",
            prompt=prompt,
